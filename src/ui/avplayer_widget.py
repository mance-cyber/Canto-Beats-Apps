...
class AVPlayerWidget(QWidget):
    ...
    def _embed_player_layer(self):
        ...
        try:
            ...
            if ns_view and hasattr(ns_view, 'setWantsLayer_'):
                ...
                # Position at bottom center (Qt flipped coordinates - origin at top-left)
                layer_height = 60
    ...
    def seek(self, time_seconds: float):
        ...
        try:
            ...
            # 使用 CMTimeMakeWithSeconds (PyObjC 原生支持)
            seek_time = CMTimeMakeWithSeconds(time_seconds, 600)
            self.player.seekToTime_(seek_time)
            logger.info(f"[AVPlayer] Seeked to {time_seconds:.1f}s")
    ...
    def load_subtitle(self, file_path: str):
        ...
        try:
            ...
            logger.info(f"Loaded {len(self._subtitles)} subtitles for overlay")
    ...
    def _update_subtitle(self, current_time: float):
        ...
        # Update CATextLayer if changed
        if current_text != self._current_subtitle:
            ...
            if self.subtitle_layer:
                try:
                    ...
                    # Fallback: just set plain string
                    self.subtitle_layer.setString_(current_text if current_text else "")
