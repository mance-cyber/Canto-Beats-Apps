...
class TimelineEditor(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Scene detection attributes
        self.scene_cuts = []  # List[SceneCut]
        self.video_path = None  # Store video path for scene detection
    ...
    def _create_toolbar(self) -> QWidget:
        ...
        # Separator for scene tools
        separator2 = QFrame()
        separator2.setFrameShape(QFrame.VLine)
        separator2.setStyleSheet("background-color: #475569; max-width: 1px;")
    ...
    def load_audio(self, audio_path: str, duration: float):
        """Load audio and generate waveform in background"""
        self.duration = duration
        ...
        # Extract waveform data in a background thread to avoid beachball
        ...
        class WaveformWorker(QThread):
            ...
            def run(self):
                try:
                    ...
                    data = AudioPreprocessor.extract_waveform_data(self.path)
                    self.finished.emit(data)
                except Exception as e:
                    print(f"Error extracting waveform in thread: {e}")
                    self.finished.emit(None)
        
        self._waveform_worker = WaveformWorker(audio_path)
    ...
    def _on_waveform_ready(self, data):
        """Callback when waveform data is ready"""
        if data is not None:
            self.waveform_track.set_waveform(data)
            self.waveform_track.update()
            print(f"Waveform data set: {len(data)} points")
    ...
    def load_video(self, video_path: str):
        ...
        try:
            ...
            # 優先使用 AVFoundation (macOS 原生)
            from utils.avf_thumbnail import get_thumbnail_extractor
            extractor = get_thumbnail_extractor()
            
            # 判斷使用的提取器類型
            extractor_name = "AVFoundation" if extractor.__name__ == "AVFoundationThumbnailExtractor" else "FFmpeg"
            logger.info(f"[Timeline] Using {extractor_name} for thumbnails")
    ...
    # ==================== Scene Detection and Alignment Methods ====================
    
    def set_video_path(self, video_path: str):
        """設置視頻路徑 (用於場景偵測)"""
        self.video_path = video_path
        self.scene_detect_btn.setEnabled(video_path is not None)
    
    def set_scene_cuts(self, scene_cuts: List):
        """設置場景切換點"""
        self.scene_cuts = scene_cuts
        
        # 傳遞給所有 track
        self.subtitle_track.set_scene_cuts(scene_cuts)
        self.video_track.set_scene_cuts(scene_cuts)
        self.waveform_track.set_scene_cuts(scene_cuts)
    ...
    def _detect_scenes(self):
        ...
        if not self.video_path:
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.warning(self, "錯誤", "未載入視頻文件")
            return
        
        # 顯示進度對話框
        from ui.animated_progress_dialog import AnimatedProgressDialog
        progress_dialog = AnimatedProgressDialog(self)
        progress_dialog.setWindowTitle("偵測場景")
        progress_dialog.set_status("正在分析視頻場景...")
        ...
        class SceneDetectionWorker(QThread):
            finished = Signal(list)  # List[SceneCut]
            ...
            def run(self):
                try:
                    ...
                    scene_cuts = detector.detect_scenes(
                        self.video_path,
                        sample_interval=0.1,
                        progress_callback=self.progress.emit
                    )
                    self.finished.emit(scene_cuts)
                except Exception as e:
                    import traceback
                    self.error.emit(str(e) + "\n" + traceback.format_exc())
        
        # 啟動 worker
        worker = SceneDetectionWorker(self.video_path)
        
        def on_progress(value):
            progress_dialog.set_progress(int(value * 100))
        
        def on_finished(scene_cuts):
            progress_dialog.close()
            self.set_scene_cuts(scene_cuts)
            ...
            QMessageBox.information(
                self,
                "場景偵測完成",
                f"偵測到 {len(scene_cuts)} 個場景切換點"
            )
        
        def on_error(error_msg):
            progress_dialog.close()
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.critical(self, "錯誤", f"場景偵測失敗:\n{error_msg}")
    ...
    def _align_subtitles_to_scenes(self):
        ...
        if not self.scene_cuts:
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.warning(self, "錯誤", "請先偵測場景切換點")
            return
        
        segments = self.track.segments
        if not segments:
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.warning(self, "錯誤", "沒有字幕可對齊")
            return
        
        # 詢問對齊模式
        from PySide6.QtWidgets import QMessageBox, QComboBox, QDialog, QVBoxLayout, QLabel, QDialogButtonBox
        
        dialog = QDialog(self)
        dialog.setWindowTitle("選擇對齊模式")
        layout = QVBoxLayout(dialog)
        
        label = QLabel("請選擇字幕對齊模式:")
        layout.addWidget(label)
        
        combo = QComboBox()
        combo.addItems(["對齊開始和結束 (nearest)", "只對齊開始 (start_only)", "只對齊結束 (end_only)"])
        ...
        mode_map = {0: 'nearest', 1: 'start_only', 2: 'end_only'}
        mode = mode_map[combo.currentIndex()]
        
        # 執行對齊
        from models.subtitle_aligner import SubtitleAligner
        
        aligner = SubtitleAligner(tolerance=0.5)
        results = aligner.align_to_scenes(segments, self.scene_cuts, mode)
        
        # 記錄到編輯歷史 (bulk operation)
        ...
        for result in results:
            if result.aligned_to_scene:
                segments[result.index]['start'] = result.aligned_start
                segments[result.index]['end'] = result.aligned_end
                aligned_count += 1
        
        # 記錄操作 (for undo)
        self.edit_history.add_operation('align_scenes', {
            'original_segments': original_segments,
            'aligned_segments': [seg.copy() for seg in segments]
        })
        ...
        QMessageBox.information(
            self,
            "對齊完成",
            f"已對齊 {aligned_count} / {len(segments)} 個字幕到場景切換點"
        )
