...
def get_current_version() -> str:
    ...
    try:
        ...
        config = Config()
        return config.get("version", "1.0.0-macOS")
    except Exception:
        return "1.0.0-macOS"


def extract_version_number(version_str: str) -> str:
    """Extract numeric version from string like '1.0.5-macOS' -> '1.0.5'"""
...
class UpdateChecker:
    ...
    def check_for_update(self) -> Tuple[bool, Optional[UpdateInfo]]:
        ...
        try:
            ...
            # Compare versions (extract numeric part only for comparison)
            current_num = extract_version_number(self.current_version)
            server_num = extract_version_number(server_version)
            
            if pkg_version.parse(server_num) > pkg_version.parse(current_num):
