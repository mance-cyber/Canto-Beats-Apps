...
class WaveformTrack(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.scene_cuts = []  # NEW: List[SceneCut]
    ...
    def paintEvent(self, event):
        ...
        # Draw scene markers
        self._draw_scene_markers(painter, start_sec, end_sec)
    ...
    def _draw_scene_markers(self, painter, start_sec, end_sec):
        """繪製場景切換標記線"""
        if not hasattr(self, 'scene_cuts') or not self.scene_cuts:
            return
            
        painter.setPen(QPen(QColor(255, 165, 0), 2))  # Orange
        for scene_cut in self.scene_cuts:
            # Handle both object with timestamp attr and float/int
            timestamp = getattr(scene_cut, 'timestamp', scene_cut)
            if isinstance(timestamp, (int, float)) and start_sec <= timestamp <= end_sec:
