...
class AVFoundationThumbnailExtractor:
    ...
    def extract_thumbnails(
        ...
        try:
            ...
            # Use kUTTypeJPEG from CoreServices instead of UniformTypeIdentifiers (Python 3.9 compatible)
            ...
            # 關鍵優化：使用大容差讓系統跳到最近的關鍵幀 (I-Frame)
            # 這是專業剪輯軟體「秒出縮略圖」的核心技術
            # kCMTimePositiveInfinity 表示「任何時間都可以」，系統會自動選擇最近的關鍵幀
            generator.setRequestedTimeToleranceBefore_(CMTimeMake(int(interval * 600), 600))
            generator.setRequestedTimeToleranceAfter_(CMTimeMake(int(interval * 600), 600))
            ...
            while current_time < duration:
                ...
                try:
                    ...
                    # Generate image using PyObjC pattern
                    # copyCGImageAtTime_actualTime_error_ returns (cgimage, actualTime) tuple
                    # error is passed by reference and returned as part of the method
                    result = generator.copyCGImageAtTime_actualTime_error_(
                        request_time, None, None
                    )
                    
                    # Handle different return value patterns from PyObjC
                    if isinstance(result, tuple):
                        if len(result) == 2:
                            cgimage, actual_time = result
                        elif len(result) == 3:
                            cgimage, actual_time, error = result
                        else:
                            cgimage = result[0] if result else None
                    else:
                        cgimage = result
