...
class SubtitleExporter:
    ...
    def export_srt(self, segments: List[Dict], output_path: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Segments count: {len(segments) if segments else 0}")
            if segments and len(segments) > 0:
                logger.error(f"First segment keys: {list(segments[0].keys()) if isinstance(segments[0], dict) else 'not a dict'}")
            return False
